GLX_Move_F = [
	...
	if (vehicle leader _group isKindOf "Air") then
	{
		...
		// _position set [2, (getPos vehicle leader _group select 2) ];
	};
	...
	if (_leader in _units) then
	{
		...
		_random = [_enemy, _group, _random] call (GLX_Move_F select 3);
		...
		// if (floor (random 100) < (GLX_Tweak select 1) ) then
