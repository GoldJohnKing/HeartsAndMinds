...
for "_id" from 0 to (count _locations - 1) do {
    ...
    if (_type in _cities) then {
        ...
        _radius = (_radius max 800) min 1200; // Edited: Change city radius, default = (_radius max 160) min 800
        ...
        */
        if ( // Edited: Add safezones
            (getMarkerPos "btc_base" inArea [_position, 300, 300, 0, false]) || 
            (getMarkerPos "safepos_1" inArea [_position, 300, 300, 0, false])
        ) exitWith {};
