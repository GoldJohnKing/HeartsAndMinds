...
if (_city getVariable ["spawn_more", false]) then {
    ...
    for "_i" from 1 to (ceil random 5) do { // Edited: Spawn many enemy static weapons every "spawn_more" city
        [[_city, random _spawningRadius] call CBA_fnc_randPos, btc_type_mg + btc_type_gl, random 360] call btc_mil_fnc_create_static;
    };
    if (btc_p_veh_armed_spawn_more) then {
        ...
        for "_i" from 1 to (1 + ceil random 3) do { // Edited: Tweak enemy armed vehicle amount, default = (1 + round random 2)
    ...
    [[_city, 250] call CBA_fnc_randPos, btc_type_motar, random 360] call btc_mil_fnc_create_static; // Edited: Spawn enemy artillery every "spawn_more" city
};
...
if (
    ...
    if (_city inArea [btc_cache_pos, _cachingRadius, _cachingRadius, 0, false]) then {
        ...
        [[_city, 250] call CBA_fnc_randPos, btc_type_motar, random 360] call btc_mil_fnc_create_static; // Edited: Spawn enemy artillery every "cache" city
    };
};
...
if (_has_ho && {!(_city getVariable ["ho_units_spawned", false])}) then {
    ...
    [[_city, 250] call CBA_fnc_randPos, btc_type_motar, random 360] call btc_mil_fnc_create_static;// Edited: Spawn enemy artillery every "hideout" city
};
...
[{
    ...
    if (_has_en) then {
        ...
        _trigger setTriggerStatements ["[thisList] call btc_p_city_free_trigger_condition", format ["[%1, thisList] call btc_city_fnc_set_clear", _id], ""];
...
if (_numberOfPatrol < _p_patrol_max) then {
    ...
    for "_i" from 1 to round _r do {
        ...
        [[_group, 2, _city, _cachingRadius + btc_patrol_area], btc_mil_fnc_create_patrol] call btc_delay_fnc_exec; // Edited: Adjust probability of AI patrol types, making it spawn more enemy vehicles, default = [_group, 1 + round random 1, _city, _cachingRadius + btc_patrol_area], btc_mil_fnc_create_patrol]
    };
};
//Traffic
