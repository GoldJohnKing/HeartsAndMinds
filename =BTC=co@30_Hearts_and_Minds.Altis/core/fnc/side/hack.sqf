...
for "_i" from 1 to (2 + round random 1) do {
    _groups pushBack ([btc_mil_fnc_send, [_closest, getPos _terminal, 1, selectRandom (btc_type_motorized + btc_type_motorized_armed)]] call CBA_fnc_directCall); // Edited: Spawn both btc_type_motorized and btc_type_motorized_armed, default = [btc_mil_fnc_send, [_closest, getPos _terminal, 1, selectRandom btc_type_motorized]]
};
...
private _fx = createVehicle ["test_EmptyObjectForFireBig", [_x, _y, _altitude], [], 0, "CAN_COLLIDE"]; // Edited: Use test_EmptyObjectForFireBig instead of test_EmptyObjectForSmoke to have better performance
