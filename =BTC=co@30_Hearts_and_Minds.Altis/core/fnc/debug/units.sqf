...
_TXTunits ctrlSetText format ["等待:%1s 单位:%2 负载均衡:%3 小队:%4 敌军巡逻:%5 平民交通:%6", [btc_delay_timeDebug, 0] select (btc_delay_timeDebug < 0.001), _count_units, _count_units - _count_units_own, count allGroups, count btc_patrol_active, count btc_civ_veh_active]; // Edited: Translate and shorten the phrases to support wider fonts, default = "DELAY:%1s UNITS:%2 NOT-ON-SERVER:%3 | GROUPS:%4 | Patrol:%5 Traffic:%6"
